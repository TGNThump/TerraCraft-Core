...
public class CommandMethod implements Comparator<Method>{
	...
	public static Optional<uk.co.terragaming.code.terracraft.CoreMechanics.CommandMechanics.Command> getParent(Method method){
		if (!hasParent(method)) return Optional.empty();
		String[] args = getParentString(method).split(" ");
		uk.co.terragaming.code.terracraft.CoreMechanics.CommandMechanics.Command command = CommandRegistry.getCommand(args);
